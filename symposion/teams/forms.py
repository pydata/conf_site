...
class TeamInvitationForm(forms.Form):
    ...
    email = forms.EmailField(
        label=_("Email"),
        help_text=_(
            "email address must be that of an account on this "
            "conference site"
        ),
    )
    ...
    def clean(self):
        ...
        try:
            ...
            raise forms.ValidationError(
                mark_safe(
                    _(
                        "no account with email address <b>%s</b> found "
                        "on this conference site"
                    )
                    % escape(email)
                )
            )
    ...
    def invite(self):
        if self.state is None:
            Membership.objects.create(
                team=self.team, user=self.user, state="invited"
            )
