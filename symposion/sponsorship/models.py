...
# "really just care about the ones we have today:
# print logo, web logo, print description, web description and the ad."
...
class Sponsor(models.Model):
    ...
    def website_logo(self):
        if self.sponsor_logo is None:
            benefits = self.sponsor_benefits.filter(
                benefit__type="weblogo", upload__isnull=False
            )[:1]
            if benefits.count():
    ...
    def listing_text(self):
        if not hasattr(self, "_listing_text"):
            ...
            benefits = self.sponsor_benefits.filter(
                benefit__name="Sponsor Description"
            )
            if benefits.count():
    ...
    def reset_benefits(self):
        ...
        if level:
            for benefit_level in level.benefit_levels.all():
                ...
                sponsor_benefit, created = SponsorBenefit.objects.get_or_create(        # noqa: E501
    ...
    def benefit_is_complete(self, name):
        ...
        if BenefitLevel.objects.filter(
            level=self.level, benefit__name=name
        ).exists():
            try:
...
def _check_level_change(sender, instance, created, **kwargs):
    if instance and (
        created or instance.level_id != instance._initial_level_id
    ):
...
CONTENT_TYPE_CHOICES = [("simple", "Simple")] + [
...
class BenefitLevel(models.Model):
    ...
    max_words = models.PositiveIntegerField(
        _("Max words"), blank=True, null=True
    )
    other_limits = models.CharField(
        _("Other limits"), max_length=200, blank=True
    )
...
class SponsorBenefit(models.Model):
    ...
    def clean(self):
        ...
        if self.max_words and num_words > self.max_words:
            raise ValidationError(
                _(
                    "Sponsorship level only allows for %(word)s words, "
                    "you provided %(num)d."
                )
    ...
    def _is_text_benefit(self):
        return self.benefit.type in ["text", "richtext", "simple"] and bool(
            self.text
        )
    ...
    def _is_complete(self):
        return self.active and (
            self._is_text_benefit() or self._is_upload_benefit()
        )
