...
def proposals_generator(request, queryset, user_pk=None, check_speaker=True):
    ...
    for obj in queryset:
        ...
        try:
            ...
            obj.user_vote_css = LatestVote.objects.get(
                **lookup_params
            ).css_class()
...
# Returns a list of all proposals, proposals reviewed by the user,
# or the proposals the user has yet to review
...
def review_list(request, section_slug, user_pk):
    ...
    proposals = proposals_generator(
        request, proposals, user_pk=user_pk, check_speaker=not admin
    )
...
def review_admin(request, section_slug):
    ...
    def reviewers():
        ...
        for team in Team.objects.filter(
            permissions__codename="can_review_%s" % section_slug
        ):
            for membership in team.memberships.filter(
                ...
                user.plus_zero = LatestVote.objects.filter(
                    user=user, vote=LatestVote.VOTES.PLUS_ZERO
                ).count()
                user.minus_zero = LatestVote.objects.filter(
                    user=user, vote=LatestVote.VOTES.MINUS_ZERO
                ).count()
    ...
    ctx = {"section_slug": section_slug, "reviewers": reviewers()}
...
def review_detail(request, pk):
    ...
    try:
        latest_vote = LatestVote.objects.get(
    ...
    return render(
        request,
        "symposion/reviews/review_detail.html",
        {
            "proposal": proposal,
...
def review_status(request, section_slug=None, key=None):
    ...
    ctx = {"section_slug": section_slug, "vote_threshold": VOTE_THRESHOLD}

    queryset = ProposalBase.objects.select_related(
        "speaker__user", "result"
    ).select_subclasses()
    if section_slug:
        queryset = queryset.filter(kind__section__slug=section_slug)

    proposals = {
        # proposals with at least VOTE_THRESHOLD reviews
        # and at least one +1 and no -1s, sorted by the 'score'
        ...
        # proposals with at least VOTE_THRESHOLD reviews
        # and at least one -1 and no +1s, reverse sorted by the 'score'
        ...
        # proposals with at least VOTE_THRESHOLD reviews
        # and neither a +1 or a -1, sorted by total votes (lowest first)
        "indifferent": queryset.filter(
            result__vote_count__gte=VOTE_THRESHOLD,
            result__minus_one=0,
            result__plus_one=0,
        ).order_by("result__vote_count"),
        # proposals with at least VOTE_THRESHOLD reviews
        # and both a +1 and -1, sorted by total votes (highest first)
        "controversial": queryset.filter(
            result__vote_count__gte=VOTE_THRESHOLD,
            result__plus_one__gt=0,
            result__minus_one__gt=0,
        ).order_by("-result__vote_count"),
        # proposals with fewer than VOTE_THRESHOLD reviews
        "too_few": queryset.filter(
            result__vote_count__lt=VOTE_THRESHOLD
        ).order_by("result__vote_count"),
    }
    ...
    for status in proposals:
        if key and key != status:
            continue
        proposals[status] = list(
            proposals_generator(
                request, proposals[status], check_speaker=not admin
            )
        )

    if key:
        ctx.update({"key": key, "proposals": proposals[key]})
    else:
        ctx["proposals"] = proposals
...
def review_assignments(request):
    ...
    return render(
        request,
        "symposion/reviews/review_assignment.html",
...
def review_bulk_accept(request, section_slug):
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            talks = ProposalBase.objects.filter(
                id__in=talk_ids
    ...
    return render(
        request, "symposion/reviews/review_bulk_accept.html", {"form": form}
    )
...
def result_notification(request, section_slug, status):
    ...
    proposals = (
        ProposalBase.objects.filter(
            kind__section__slug=section_slug, result__status=status
        )
        .select_related("speaker__user", "result")
        .select_subclasses()
    )
...
def result_notification_prepare(request, section_slug, status):
    ...
    return render(
        request, "symposion/reviews/result_notification_prepare.html", ctx
    )
...
def result_notification_send(request, section_slug, status):
    ...
    return redirect(
        "result_notification", section_slug=section_slug, status=status
    )
