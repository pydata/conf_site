...
def score_expression():
    return (3 * F("plus_one") + F("plus_zero")) - (
...
class Votes(object):
    ...
    CHOICES = [
        (
            PLUS_ONE,
            _("+1 — Good proposal and I will argue for it to be accepted."),
        ),
        (
            PLUS_ZERO,
            _("+0 — OK proposal, but I will not argue for it to be accepted."),
        ),
        (
            MINUS_ZERO,
            _(
                "−0 — Weak proposal, "
                "but I will not argue strongly against acceptance."
            ),
        ),
        (
            MINUS_ONE,
            _("−1 — Serious issues and I will argue to reject this proposal."),
        ),
    ]
...
class ReviewAssignment(models.Model):
    ...
    assigned_at = models.DateTimeField(
        default=timezone.now, verbose_name=_("Assigned at")
    )
    ...
    def create_assignments(cls, proposal, origin=AUTO_ASSIGNED_INITIAL):
        speakers = [proposal.speaker] + list(
            proposal.additional_speakers.all()
        )
        reviewers = (
            ...
            .filter(groups__name="reviewers")
            .filter(
                Q(reviewassignment__opted_out=False) | Q(reviewassignment=None)
            )
            .annotate(num_assignments=models.Count("reviewassignment"))
            .order_by("num_assignments", "?")
        )
        num_assigned_reviewers = ReviewAssignment.objects.filter(
            proposal_id=proposal.id, opted_out=0
        ).count()
        for reviewer in reviewers[
            : max(0, cls.NUM_REVIEWERS - num_assigned_reviewers)
        ]:
...
class ProposalMessage(models.Model):
    ...
    submitted_at = models.DateTimeField(
        default=timezone.now, editable=False, verbose_name=_("Submitted at")
    )
...
class Review(models.Model):
    ...
    vote = models.CharField(
        max_length=2, blank=True, choices=VOTES.CHOICES, verbose_name=_("Vote")
    )
    ...
    def save(self, **kwargs):
        ...
        if self.vote:
            vote, created = LatestVote.objects.get_or_create(
                ...
                user=self.user,
                defaults=dict(vote=self.vote, submitted_at=self.submitted_at),
            )
            if not created:
    ...
    def delete(self):
        ...
        user_reviews = model._default_manager.filter(
            proposal=self.proposal, user=self.user
        )
        try:
            # find the latest review
            latest = user_reviews.exclude(pk=self.pk).order_by(
                "-submitted_at"
            )[0]
        except IndexError:
            ...
            if self == latest:
                ...
                # previous vote
                previous = user_reviews.filter(
                    submitted_at__lt=self.submitted_at
                ).order_by("-submitted_at")[0]
                self.proposal.result.update_vote(
                    self.vote, previous=previous.vote, removal=True
                )
                lv = LatestVote.objects.filter(
                    proposal=self.proposal, user=self.user
                )
                lv.update(
                    vote=previous.vote, submitted_at=previous.submitted_at
                )
            else:
                # self is not the latest review so we just need to decrement
                # the comment count
                self.proposal.result.comment_count = (
                    models.F("comment_count") - 1
                )
...
class LatestVote(models.Model):
    ...
    # like some complicated encoding system.
    vote = models.CharField(
        max_length=2, choices=VOTES.CHOICES, verbose_name=_("Vote")
    )
    submitted_at = models.DateTimeField(
        default=timezone.now, editable=False, verbose_name=_("Submitted at")
    )
...
class ProposalResult(models.Model):
    ...
    vote_count = models.PositiveIntegerField(
        default=0, verbose_name=_("Vote count")
    )
    plus_one = models.PositiveIntegerField(
        default=0, verbose_name=_("Plus one")
    )
    plus_zero = models.PositiveIntegerField(
        default=0, verbose_name=_("Plus zero")
    )
    minus_zero = models.PositiveIntegerField(
        default=0, verbose_name=_("Minus zero")
    )
    minus_one = models.PositiveIntegerField(
        default=0, verbose_name=_("Minus one")
    )
    accepted = models.NullBooleanField(
        choices=[(True, "accepted"), (False, "rejected"), (None, "undecided")],
        default=None,
    ...
    def full_calculate(cls):
        for proposal in ProposalBase.objects.all():
            result, created = cls._default_manager.get_or_create(
                proposal=proposal
            )
            result.comment_count = Review.objects.filter(
                proposal=proposal
            ).count()
            result.vote_count = LatestVote.objects.filter(
                proposal=proposal
            ).count()
            result.plus_one = LatestVote.objects.filter(
                proposal=proposal, vote=VOTES.PLUS_ONE
            ).count()
            result.plus_zero = LatestVote.objects.filter(
                proposal=proposal, vote=VOTES.PLUS_ZERO
            ).count()
            result.minus_zero = LatestVote.objects.filter(
                proposal=proposal, vote=VOTES.MINUS_ZERO
            ).count()
            result.minus_one = LatestVote.objects.filter(
                proposal=proposal, vote=VOTES.MINUS_ONE
            ).count()
            result.save()
            cls._default_manager.filter(pk=result.pk).update(
    ...
    def update_vote(self, vote, previous=None, removal=False):
        ...
        if previous:
            ...
            if removal:
                setattr(
                    self, mapping[previous], models.F(mapping[previous]) + 1
                )
            else:
                setattr(
                    self, mapping[previous], models.F(mapping[previous]) - 1
        ...
        self.save()
        model = self.__class__
        model._default_manager.filter(pk=self.pk).update(
            score=score_expression()
        )
...
class Comment(models.Model):
    ...
    public = models.BooleanField(
        choices=[(True, _("public")), (False, _("private"))],
        default=False,
        verbose_name=_("Public"),
    )
    commented_at = models.DateTimeField(
        default=timezone.now, verbose_name=_("Commented at")
    )
