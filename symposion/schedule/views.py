...
def schedule_list(request, slug=None):
    ...
    ctx = {"schedule": schedule, "presentations": presentations}
...
def schedule_list_csv(request, slug=None):
    ...
    response["Content-Disposition"] = (
        'attachment; filename="%s.csv"' % file_slug
    )
...
def schedule_edit(request, slug=None):
    ...
    days_qs = Day.objects.filter(schedule=schedule)
    days = [TimeTable(day) for day in days_qs]
    ctx = {"schedule": schedule, "days": days, "form": form}
...
def schedule_slot_edit(request, slug, slot_pk):
    ...
    if request.method == "POST":
        ...
        ctx = {"slug": slug, "form": form, "slot": slot}
...
def schedule_json(request):
    ...
    return HttpResponse(
        json.dumps({"schedule": data}), content_type="application/json"
    )
...
def session_list(request):
    ...
    return render(
        request, "symposion/schedule/session_list.html", {"sessions": sessions}
    )
...
def session_detail(request, session_id):
    ...
    chairs = SessionRole.objects.filter(
        session=session, role=SessionRole.SESSION_ROLE_CHAIR
    ...
    runners = SessionRole.objects.filter(
        session=session, role=SessionRole.SESSION_ROLE_RUNNER
    ).exclude(status=False)
    if runners:
    ...
    if request.method == "POST" and request.user.is_authenticated():
        if (
            ...
            or not request.user.profile.is_complete
        ):
    ...
    return render(
        request,
        "symposion/schedule/session_detail.html",
        {
            "session": session,
            "chair": chair,
